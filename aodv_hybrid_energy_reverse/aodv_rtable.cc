...
u_int16_t
aodv_rtable::total_rts(nsaddr_t id) { //total routes
  int j=0;
  aodv_rt_entry *rt = rthead.lh_first;
  for(; rt; rt = rt->rt_link.le_next) {
...
aodv_rt_entry*
aodv_rtable::rt_current(nsaddr_t id, u_int16_t order) { //routes affiliated with the selected
aodv_rt_entry *rt = rthead.lh_first;
  for(; rt; rt = rt->rt_link.le_next) {
...
void
aodv_rtable::order_routes(nsaddr_t id) { //to set route orders
  int j=0;
  aodv_rt_entry *rt = rthead.lh_first;
  for(; rt; rt = rt->rt_link.le_next) {
    if ((rt->rt_dst == id) && (rt->rt_flags==RTF_UP)) {
