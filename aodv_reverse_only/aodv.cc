...
static int ord;
static int thackers;
static int hack_packets;
static hackNode hackerNode[100];
...
AODV::command(int argc,
  ...
  if (argc == 2) {
    ...
    //modify start
      if (strncasecmp(argv[1], "hacker", 7) == 0) {
        hacker_add(index);
        return TCL_OK;
      }
    //modify end
...
//modify start
  /*
     Hacker List Management  Functions
  */

  void
  AODV::hacker_add(nsaddr_t id) {
    thackers=thackers+1;
    hackerNode[thackers].nodeID=id;
    hackerNode[thackers].htype=1;
    hackerNode[thackers].hindex=thackers; 
  }

  bool 
  AODV::hacker_lookup(nsaddr_t id){
    for (int i=1;i<=thackers;i++) {
      if (hackerNode[i].nodeID==id)
        return true;
    }
    return false;
}
...
AODV::rt_down(aodv_rt_entry *rt) {
  ...
  //modify start
    rt->rt_order=6;
    rtable.rt_lastorder(rt->rt_dst, 0);
  //modify end
...
//modify start
  ...
  */
  ...
  AODV::ht_print(void) {
    FILE *out_file;
    char nowfile[50]="hlog.txt";
    out_file=fopen(nowfile,"w");
    aodv_rt_entry *rt;
    fprintf(out_file,"Total Hacked: %d\n", hack_packets);
    fclose(out_file);
  }
//modify end
...
AODV::rt_resolve(Packet *p) {
  ...
  //modify start
    ...
    // }
    if (hacker_lookup(index)) {
      // Packet::free(p);
      // return;
      hack_packets = hack_packets + 1;
      ht_print();
    }
  //modify end
  ...
  //modify start
    if (rt && (ih->saddr() == index) && (rtable.total_rts(rt->rt_dst)!=0)) {
      ...
      // =============== Hopping =======================    
      //    lastorder=(rt->rt_last+1)%(rtable.total_rts(rt->rt_dst));
      //    rtable.rt_lastorder(rt->rt_dst, lastorder);
      //    rt = rtable.rt_hopping(rt->rt_dst);
    }
    if(rt == 0) {
      rt = rtable.rt_add(ih->daddr());
    }
...
AODV::recvAODV(Packet *p) {
  ...
  //modify start
    ...
    #ifdef DEBUG
      fprintf(stderr,"Checking packet type started\n");
    #endif // DEBUG
...
AODV::recvRequest(Packet *p) {
  ...
  if (id_lookup(rq->rq_src, rq->rq_bcast_id)) {
    ...
    #ifdef DEBUG
    fprintf(stderr, "%s: discarding request (heard it before)\n", __FUNCTION__);
    #endif // DEBUG
  ...
  if (rq->rq_dst == index) {
    ...
    #ifdef DEBUG
    fprintf(stderr, "%d - %s: destination starting Request Reply\n",
      index, __FUNCTION__);
    #endif // DEBUG
...
AODV::recvReverse(Packet *p) {
  ...
  //modify start
    if (rp->rp_src == index) {
      #ifdef DEBUG
        fprintf(stderr, "%s: Got my own RQREP\n", __FUNCTION__);
      #endif // DEBUG
      Packet::free(p);
      return;
    }
  ...
  //I am destination
    ...
    if ( (rt0->rt_seqno < rp->rp_dst_seqno ) ||   // newer route 
      ...
      #ifdef MDEBUG
        fprintf(stderr, "Adding new router or updating excisting route: %i\n",rt0->rt_order);
      #endif // DEBUG
      ...
      #ifdef MDEBUG
        fprintf(stderr, "Adding rotue No. %i\n",rtable.total_rts(rp->rp_src)+1);
      #endif // DEBUG
      rt0=rtable.rt_add(rp->rp_src);
      rt_update(rt0, rp->rp_dst_seqno, rp->rp_hop_count,ih->saddr(), max(rt0->rt_expire, (CURRENT_TIME + REV_ROUTE_LIFE)));    
      if (rt0->rt_req_timeout > 0.0 ) {
...
AODV::sendReverse(nsaddr_t dst) {
  ...
  //modify start
    ...
    // #endif // DEBUG
    // assert(rt);
      fprintf(stderr, "(%2d) - %2d sending Reverse Request, dst: %d, tout %f ms\n",     ++route_request,  index,  dst,  CURRENT_TIME);
    #endif // DEBUG
